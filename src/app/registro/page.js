  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-100 p-4">
      <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-lg">
        <h1 className="text-2xl font-bold text-slate-900 mb-6 text-center border-b pb-4">Registro Fiscal</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input name="rfc" value={formData.rfc} maxLength={13} required className={`${inputStyle} font-mono`} onChange={handleChange} />
            </div>
            <div>
          ...
          <button 
            type="submit" 
            disabled={loading}
            className={`w-full ${loading ? 'bg-slate-400' : 'bg-slate-900 hover:bg-black cursor-pointer'} text-white p-3 rounded-lg font-bold transition-all mt-4`}
          >
